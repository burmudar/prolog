...
func setupTest(t *testing.T, fn func(c *Config)) (client api.LogClient, cfg *Config, tearDown func()) {
	t.Helper()
...
func testProduceConsume(t *testing.T, client api.LogClient, cfg *Config) {
	...
	consume, err := client.Consume(ctx, &api.ConsumeRequest{Offset: produceResp.Offset})
	require.NoError(t, err)
